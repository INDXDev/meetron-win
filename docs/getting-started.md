...
非エンジニアの方は、最初に[READMEのAIアシスタントによる初回導入](../README.md#aiアシスタントによる初回導入推奨)にある依頼文をローカル操作対応のAIへ渡してください。このガイドのコマンドはAIが実行し、管理者認証、再起動、Chrome拡張の読み込み、ログインだけを利用者が担当する進め方を推奨します。
...
Chrome公式ビルドでは開発版拡張の読込をコマンドだけで完結できないため、ディレクトリ選択は手動です。ポップアップに`ローカルホスト未接続`と表示された場合は、コピーアイコンからコマンドを取得して実行します。コマンドはChrome設定から読み込み元の`extension`ディレクトリを探し、その親にあるリポジトリへ自動的に移動します。
...
MeetのカメラUIを安全に判定できない場合、自動参加は停止します。専用Chromeでカメラがオフになっていることを確認し、手動で参加または参加リクエストを行ってください。その後の入室検出とマイク制御は継続します。
